...
import { Image as ImageType } from '@/models/roon';
...
const HotelPhotoGallery: React.FC<HotelPhotoGalleryProps> = ({ photos }) => {
    const [currentPhotoIndex, setCurrentPhotoIndex] = useState(0)
    const [showModal, setShowModal] = useState(false)

    const openModalhandle = (index: number) => {
    ...
    const maximumVisiablePhoto = 3
    const totalPhotos = photos.length
    const displayPhotos = photos.slice(1, maximumVisiablePhoto - 1)
    const remainingPhotosCount = totalPhotos - maximumVisiablePhoto
        ...
        <div className='grid md:grid-cols-2 relative gap-5 px-3 '>
            <div className='h-[540px] relative rounded-2xl overflow-hidden'>
                <div className='hidden md:flex justify-center items-center w-full h-full'>
                    <Image
                        ...
                        width={150}
                        height={150}
                        onClick={openModalhandle.bind(this, 0)}
                    />
                </div>
            ...
            <div className='hidden md:grid grid-cols-2 h-full gap-5'>
                {displayPhotos.map((photo, idx) => (
                    <div key={idx} className='cursor-pointer h-64 rounded-2xl overflow-hidden'>
                        <Image src={photo.url} alt={`Room Photo ${idx + 2}`} width={150} height={150} className='img scale-animation' />
                    </div>
                ))}
                {remainingPhotosCount > 0 && (
                    <div
                        className='cursor-pointer relative h-64 rounded-2xl overflow-hidden'
                        onClick={openModalhandle.bind(this, maximumVisiablePhoto)}
                    >
                        <Image
                            width={150}
                            height={150}
                            src={photos[maximumVisiablePhoto - 1].url}
                            alt={`Room Photo ${maximumVisiablePhoto}`}
                            className='img'
                        />
                ...
                <div className='fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-90 z-[55]' >
                    <div className='h-[75vh] w-[320px] md:w-[700px] relative'>
                        ...
                        <button
                            ...
                            onClick={closeModal}
                        >
                            <MdCancel className='font-medium text-2xl text-tertiary-dark animate-pulse duration-700' />
                        </button>
                    </div>
                </div>
            )}
